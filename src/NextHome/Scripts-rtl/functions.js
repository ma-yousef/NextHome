...
============================================== */
$(function () {
    $('.timeline-slider').toArray().forEach(function (slider) {
        var wrapper = $(slider).find('.owl-wrapper')[0];
        var items = $(wrapper).find('.owl-item').toArray();
        if (items.length > 1) {
            $(wrapper).css('left', (parseFloat($(items[0]).css('width')) * (items.length - 1)).toString() + 'px');
        }
    });
})
